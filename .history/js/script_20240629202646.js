...
(() => {
	...
	// Puts hard coded employees into classes
	...
	const renderEmployees = () => {
		...
		employeeArray.forEach((employee) => {
			...
			row.innerHTML = `
                ...
                <td>
                    <button class="edit" data-name="${
											employee.name
										}">Edit</button>
                    <button class="delete" data-name="${
											employee.name
										}">Delete</button>
                </td>
            `;
	...
	const saveEditedEmployee = () => {
		...
		if (validateInput(name, age, hours, payRate)) {
			const employee = employeeArray.find((employee) => employee.name === name);
			if (hours >= 40 && !(employee instanceof Manager)) {
				// Change to Manager class
				employeeArray.splice(
					employeeArray.indexOf(employee),
					1,
					new Manager(name, age, hours, payRate)
				);
	...
	const deleteEmployee = (name) => {
		employeeArray = employeeArray.filter((employee) => employee.name !== name);
		saveEmployees();
		renderEmployees();
	};
